...
class Ip2LocationIpV6 extends AbstractProvider implements Provider, FileAwareProvider
{
    use FileAwareTrait;

    const DEFAULT_PATH = 'data/IP2LOCATION-LITE-DB11.IPV6.BIN';
    ...
    public function getSupportedFields()
    {
        return [
            ...
            LocationProvider::STREET_NAME,
            LocationProvider::STREET_NUMBER,
    ...
    public function geocode($address)
    {
        if (! filter_var($address, FILTER_VALIDATE_IP)) {
            throw new UnsupportedOperation('The Ip2LocationIpV6 provider does not support street addresses, only IP addresses.');
        }
        ...
        if (! isset($result['countryCode']) || $result['countryCode'] === 'Invalid IP address.') {
            throw new UnsupportedOperation(sprintf('The Ip2LocationIpV6 provider does not support IPv4 "%s".', $address));
        }
